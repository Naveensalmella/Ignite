...
export default function ExerciseDetail({ ex, color, onComplete, showComplete }) {
  return (
    <div className="ex-detail fade-in">
      {/* Visual Demo */}
      <ExerciseVisual type={ex.anim || "push"} bodyPart={ex.bodyPart} size="md" />
          ...
          {ex.steps.map((s, i) => (
            <div key={i} style={{ display: "flex", gap: 12, marginBottom: 10, fontSize: 14, lineHeight: 1.6 }}>
              <span style={{
                ...
                color, fontSize: 12, fontWeight: 700, fontFamily: "Rajdhani,sans-serif", flexShrink: 0,
              }}>{i + 1}</span>
        ...
        <div style={{ marginTop: 14 }}>
          ...
          {ex.tips.map((t, i) => (
            <div key={i} style={{
              ...
              paddingLeft: 12, borderLeft: "2px solid rgba(251,191,36,.2)",
            }}>{t}</div>
          ))}
        </div>
      )}
      ...
      {showComplete && (
        <button className="bp" onClick={onComplete} style={{ width: "100%", marginTop: 16, padding: 14, letterSpacing: 1 }}>
          ✓ COMPLETE
        </button>
      )}
    </div>
