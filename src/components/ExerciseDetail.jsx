...
export default function ExerciseDetail({ex,color,onComplete,showComplete}){
  ...
  return(<div className="ex-detail fade-in">
    <div style={{display:"flex",gap:16,alignItems:"flex-start",marginBottom:16}}>
      ...
      <div style={{flex:1}}>
        ...
        <div style={{display:"flex",gap:10,marginTop:4,flexWrap:"wrap"}}>
          <span style={{fontSize:12,color,fontWeight:600}}>{ex.reps}</span>
          {ex.muscle&&<span style={{fontSize:12,color:"#6b7280"}}>· {ex.muscle}</span>}
          <span style={{fontSize:12,color:"#fbbf24"}}>~{ex.cal} cal</span>
        </div>
      </div>
    </div>
    <div className="sl" style={{color,marginTop:12}}>How To Perform</div>
    {ex.steps?.map((s,i)=>(<div key={i} style={{display:"flex",gap:12,marginBottom:10,fontSize:14,lineHeight:1.6}}>
    ...
    {ex.tips&&ex.tips.length>0&&(<>
      ...
      {ex.tips.map((t,i)=>(<div key={i} style={{fontSize:13,color:"#6b7280",marginBottom:6,paddingLeft:12,borderLeft:"2px solid rgba(251,191,36,.2)"}}>
        {t}
      </div>))}
    </>)}
    {showComplete&&<button className="bp" onClick={onComplete} style={{width:"100%",marginTop:16,padding:14,letterSpacing:1}}>✓ COMPLETE THIS EXERCISE</button>}
  </div>);
}
