...
export default function Nutrition({ foodLog, setFoodLog, addXP, profile }) {
  ...
  const addFood = () => {
    ...
    const entry = { id: Date.now(), name: selected.name, cat: selected.cat, grams, meal, ...nutr,
  ...
  const addManual = () => {
    ...
    const entry = { id: Date.now(), name: manual.name, cat: "Custom", grams: parseFloat(manual.grams)||0, meal,
  ...
  const quickAdd = (f) => {
    ...
    if (dbFood) {
      ...
      // Re-add from history
      const entry = { id: Date.now(), name: f.name, cat: f.cat||"Custom", grams: f.grams||100, meal: defaultMeal(),
